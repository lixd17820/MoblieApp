...
//        });
    ...
    private View.OnClickListener modZdgzItem = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            Iterator<String> it = GlobalSystemParam.bjVehSet.iterator();
            while (it.hasNext()) {
                String s = it.next();
                list.add("1" + "," + s);
                data.add(s);
            }
            it = GlobalSystemParam.bjCbzSet.iterator();
            while (it.hasNext()) {
                String s = it.next();
                list.add("2" + "," + s);
                data.add(s);
                    ...
                    .itemsCallbackMultiChoice(sels, new MaterialDialog.ListCallbackMultiChoice() {
                        ...
                        public boolean onSelection(MaterialDialog dialog, Integer[] which, CharSequence[] text) {
                            if (which.length < choose.size()) {
                                GlobalSystemParam.bjVehSet.clear();
                                GlobalSystemParam.bjCbzSet.clear();
                                for (int i = 0; i < which.length; i++) {
                                    String txt = choose.get(which[i]);
                                    if (txt.startsWith("1"))
                                        GlobalSystemParam.bjVehSet.add(txt.substring(2));
                                    if (txt.startsWith("2"))
                                        GlobalSystemParam.bjCbzSet.add(txt.substring(2));
                                }
                                saveSetIntoTxt();
                                showZdgzInfo();
                            }
    ...
    OnItemClickListener itemClick = new OnItemClickListener() {
        ...
        public void onItemClick(View view, int position) {
            Bjbd bd = bjbdList.get(position).getBean();
            bd.setYdbj(1);
            bjbdBox.put(bd);
                        ...
                        public boolean onSelection(MaterialDialog dialog, Integer[] which, CharSequence[] text) {
                            if (which != null && which.length > 0) {
                                ...
                                if (index == 0 || which.length == 2)
                                    GlobalSystemParam.bjVehSet.add(bdjg.getHphm());
                                if (index == 1 || which.length == 2)
                                    GlobalSystemParam.bjCbzSet.add(bdjg.getCbz());
                                saveSetIntoTxt();
                                showZdgzInfo();
                            }
                            return true;
    ...
    private void saveSetIntoTxt() {
        ...
        try {
            obj.put("vehs", GlobalMethod.join(GlobalSystemParam.bjVehSet, ","));
            obj.put("cbzs", GlobalMethod.join(GlobalSystemParam.bjCbzSet, ","));
            GlobalSystemParam.putSavedInfo(self, "zdgz", obj.toString());
    ...
    private void showZdgzInfo() {
        ...
        if (GlobalSystemParam.bjVehSet.isEmpty() && GlobalSystemParam.bjCbzSet.isEmpty()) {
            ...
            info += "重点关注：";
            if (!GlobalSystemParam.bjVehSet.isEmpty()) {
                info += "\n车辆：" + GlobalMethod.join(GlobalSystemParam.bjVehSet, "，");
            }
            if (!GlobalSystemParam.bjCbzSet.isEmpty()) {
                info += "\n通过地点：" + GlobalMethod.join(GlobalSystemParam.bjCbzSet, "，");
            }
        }
    ...
    private List<SelectObjectBean<Bjbd>> getBjbdData(int page) {
        ...
        boolean isBig = count > needCount;
        long limit = isBig ? pageItems : (count % pageItems);
        long offset = isBig ? count - needCount : 0;
        List<Bjbd> bds = bjbdBox.query().build().find(offset, limit);
        for (Bjbd bd : bds) {
    ...
    public void eventBusMain(Bjbd bjbd) {
        ...
        if (!GlobalMethod.isRecBjbd(bjbd))
            return;
